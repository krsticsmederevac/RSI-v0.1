...
st.set_page_config( page_title = "Scaner", page_icon = "📊",layout="centered",initial_sidebar_state="auto", menu_items = {"About": "krsticsmederevac@gmail.com"})
...
tab1, tab2 ,tab3 = container.tabs(["📋 RSI and Price Change %", "📋 EMAs", "📋 MAs"])
...
#     dt['Change'] = round(dt['change'],2).astype(str) + '%'
    ...
    dt1 = dt1.style.apply(lambda x: ["background: red" if v > x.iloc[0] else "background: green" for v in x], axis = 1)
    dt2 = dt2.style.background_gradient(cmap = 'RdYlGn',subset = ['Change %'] )
